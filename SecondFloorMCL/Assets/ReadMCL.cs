...
public class ReadMCL : MonoBehaviour {
    ...
    void Start()
    {
        ...
        string obstaclefind = " "; //intialize a string 
        for (int wallnumber = 0; wallnumber < WallsList.Count; wallnumber++) //loops through all walls in wallslist
        {
            ...
            string oldobstaclefind = obstaclefind; //comparing obstacleid with old obstacleid

            if (cwall.roomid != null) //if wall is part of a room
            {
            ...
            if (cwall.obstacleid != null) //if wall is part of an obstacle
            {
                obstaclefind = cwall.obstacleid;
                if (oldobstaclefind != obstaclefind) //if obstacleid for current wall is not the same as that of the previous wall
                {
                    GameObject obstacleparent = new GameObject(obstaclefind); //make an empty GameObject with the same name as the obstacleid for current wall
                    go.transform.parent = obstacleparent.transform; //parenting
                }
                else //if obstacleid for current wall is the same as that of the previous wall
                {
                    go.transform.parent = GameObject.Find(oldobstaclefind).transform; //wall belongs to already created GameObject which parents all the walls within the current obstacleid
                }
            }
        }
    }
        ...
        foreach (XmlNode transformInfo in transformList)
        {
            ...
            foreach (XmlNode transformItems in transformcontent)
            {
                ...
                foreach (XmlNode transformItems2 in transformcontent2)
                {
                    ...
                    if (transformItems2.Name == "ObstacleName") { w.obstacleid = transformItems2.InnerText; }
